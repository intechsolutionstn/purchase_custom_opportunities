...
class ProductCODFees(models.Model):
	...
	def min_max_calculation_product(self):
		cod_acq = self.env['ir.model.data'].xmlid_to_object('cash_on_delivery_odoo_av.payment_acquirer_cod')
		for p in self:
	...
	def update_fees(self):
		cod_obj = self.env['ir.model.data'].xmlid_to_object('cash_on_delivery_odoo_av.payment_acquirer_cod')
		for p in self:
